...
if [[ $STDOUTPUT =~ $ER ]];
then
    echo "{ \"changed\": false, \"failed\": true, \"rc\": 1, \"msg\": \"\", \"stderr\": {"$STDOUTPUT"}, \"stdout\": \"\", \"stdout_lines\": \"\" }"
    exit 1
else
    echo "{ \"changed\": false, \"failed\": false, \"rc\": 0, \"msg\": \"\", \"stderr\": \"\", \"stdout\": {"$STDOUTPUT"}, \"stdout_lines\": {"$STDOUTPUT"} }"
    exit 0
fi
